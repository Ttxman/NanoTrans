...
namespace NanoTrans
{
    ...
    internal class WaveInBuffer : IDisposable
    {
        ...
        {
            if (m_Recording)
                m_Recording = m_RecordEvent.WaitOne(250, true);
            else
                Thread.Sleep(0);
        }
    ...
    /// <summary>
    /// trida starajici se o nahravani audio dat ve zvolenem formatu a pomoci vybraneho zarizeni
    /// </summary>
    public class MyWaveRecorder : IDisposable
    {
        private IntPtr m_WaveIn;
        ...
        /// <summary>
        /// vraci pocet zarizeni pro zaznam
        /// </summary>
        public static int DeviceCountIN
        {
            get { return WaveNative.waveInGetNumDevs(); }
        }

        /// <summary>
        /// vraci seznam vsech vstupnich zarizeni
        /// </summary>
        public static string[] DeviceNamesIN
        {
            get
            {
                try
                {
                    int pDevicesCount = DeviceCountIN;
                    string[] pDevicesString = new string[pDevicesCount];
                    WaveNative.WAVEINCAPS WICaps = new WaveNative.WAVEINCAPS();
                    for (int i = 0; i < pDevicesCount; i++)
                    {
                        WaveNative.waveInGetDevCaps(i, ref WICaps, (int)Marshal.SizeOf(WICaps));
                        pDevicesString[i] = WICaps.szPname;
                    }

                    return pDevicesString;
                }
                catch
                {
                    return null;
        ...
        public MyWaveRecorder(int device, WaveFormat format, int bufferSize, int bufferCount, BufferDoneEventHandler doneProc)
        {
            m_DoneProc = doneProc;
